...
def count_target_usage(
    ...
    logger.info(
        "target each class usage: {} ".format(
            per_class_select_num / per_class_num[: len(per_class_select_num)]
        )
    )
    if len(per_class_num) != len(per_class_select_num):
        logger.info(
            "target last {} class usage is 0%".format(
                len(per_class_num) - len(per_class_select_num)
            )
        )
    ...
    logger.info(
        "match right rate: {}".format(
            (target_labels == source_labels).int().sum() / len(target_labels)
        )
    )
...
def generate_new_dataset(
        ...
        if (with_pseudo_label_filter and mask) or not with_pseudo_label_filter:
            ...
            train_set.append(
                (
                    ...
                    (label, target_pseudo_label[t_idx].item()),
                    camid,
                    trackid,
                    (curidx, t_idx),
                )
    ...
    count_target_usage(
        logger,
        torch.tensor(new_targetidx),
        label_memory1,
        label_memory2,
        img_num1,
        img_num2,
        source_idxs=torch.tensor(new_target_knnidx),
    )
    ...
    for idx, data in enumerate(tqdm(source_dataset)):
        ...
        if (with_pseudo_label_filter and mask) or not with_pseudo_label_filter:
            ...
            train_set.append(
                (
                    (s_img_path, t_img_path),
                    (label, target_pseudo_label[curidx].item()),
                    camid,
                    trackid,
                    (s_idx, curidx.item()),
                )
    ...
    count_target_usage(
        logger,
        torch.tensor(new_source_knnidx),
        label_memory1,
        label_memory2,
        img_num1,
        img_num2,
        source_idxs=torch.tensor(new_source_idx),
    )
...
def do_train_uda(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if (epoch - 1) % update_epoch == 0:
            feat_memory1, feat_memory2, label_memory1, label_memory2 = update_feat(
                cfg,
                epoch,
                model,
                train_loader1,
            ...
            target_label, knnidx, knnidx_topk, target_knnidx = compute_knn_idx(
                logger,
                model,
                train_loader1,
                train_loader2,
            ...
            train_loader = generate_new_dataset(
                cfg,
                logger,
                label_memory2,
                s_dataset,
                ...
                img_num1,
                img_num2,
                with_pseudo_label_filter=cfg.SOLVER.WITH_PSEUDO_LABEL_FILTER,
            )
    ...
    if torch.cuda.device_count() > 1:
        model.module.load_param_finetune(
            os.path.join(cfg.OUTPUT_DIR, cfg.MODEL.NAME + "_best_model.pth")
        )
    else:
    ...
    for n_iter, (img, vid, camid, camids, target_view, _) in enumerate(val_loader):
        with torch.no_grad():
            ...
            if cfg.MODEL.TASK_TYPE == "classify_DA":
                feats = model(
                    img, img, cam_label=camids, view_label=target_view, return_logits=True
                )
...
def do_inference_uda(cfg, model, val_loader, num_query):
    device = "cuda"
    ...
    for n_iter, (img, vid, camid, camids, target_view, img_path) in enumerate(val_loader):
        with torch.no_grad():
            ...
            output_prob.append(probs[1])
            img_path_list.extend(img_path)
    output_prob = torch.cat(output_prob, dim=0)
    _, predict = torch.max(output_prob, 1)
    output_file_path = os.path.join(cfg.OUTPUT_DIR, "predict.txt")
    with open(output_file_path, "w") as file:
        for idx in range(len(img_path_list)):
            file.write(f"{img_path_list[idx]}\t{predict[idx].item()}\n")
    print("=========== finish image label ===========")
